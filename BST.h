...
public:
  ...
  void printInoder() const
  {
      ...
      while(!curr->isLeftThread())
		  curr = curr->left();  

      while (!current->isLeftThread())
      {
          visit(curr);
          if (curr->isRightTrhead())
          {
              curr = curr ->right();
          }
          else
          {
              curr = curr->right();
              while (curr != NULL && !curr->isLeftThread())
				  curr = curr->left();
          }
      }
  }
  ...
  void printReverse()const
  {
	  ...
	  //Step 1: Go to the rightmost node
      while(!curr->isRightThread())
          curr = curr->right();

	  //Step 2: Traverse the tree in reverse order
      while (curr != = NULL)
      {
          visit(curr);

		  //Step 3: If the current node is a left thread, follow it
          ...
          {
              curr = curr->left();
              while (curr != NULL && !curr->isRightThread())
                  curr = curr->right();
		  }
...
void BST<Key, E>::
visit(BSTNode<Key,E>* r) const {
    cout << r->key() << "-" << r->element() << endl;
}
